...
import { getFarmers, deleteFarmer } from '../../services/farmers'
  ...
  useEffect(() => {
    const fetchFarmers = async () => {
      ...
      try {
        ...
        dispatch({ type: ACTIONS.SET_FARMERS, payload: data })
      } catch (err) {
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredFarmers.map((farmer) => (
                <tr key={farmer.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500">{farmer.village}</td>
                  <td className="px-6 py-4 text-sm text-gray-500">{farmer.farm_size} acres</td>
